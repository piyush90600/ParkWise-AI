(function () {
  ...
  const isQueue = page === 'owner-verifications.html'; const isDetail = page === 'verification-detail.html';
  header.className = 'top-header shared-header';
  header.innerHTML = `<div class="page-title"><h1>${isDetail ? 'Verification Review' : isQueue ? 'Owner Verifications' : 'Admin Dashboard'}</h1><p>${isDetail ? 'Review the owner and business documents before making a decision.' : isQueue ? 'Review documents and approve eligible parking owners.' : 'Review owner applications and monitor platform activity.'}</p></div><div class="shared-profile"><button class="notification-btn" type="button" aria-label="Notifications"><i class="fa-regular fa-bell"></i></button><span class="profile-avatar">A</span><span class="profile-name">Admin</span></div>`;
}());
